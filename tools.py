...
유사도기준 = 0.70  # 실측 근거: 규정집에 있는 질문 0.76↑ / 없는 질문 0.67↓ — 완충지대의 중간
...
def 규정검색(질문: str, 개수: int = 3) -> list[dict] | dict:
    ...
    결과 = [
        ...
        for i in 상위
        if 유사도[i] >= 유사도기준  # 기준 미달 조각은 버림 (WHERE 절)
    ]
    if not 결과:  # 전부 미달 = 규정집 밖의 질문
        return {"안내": "기재요령(규정집)에서 관련 규정을 찾지 못했습니다"}
    return 결과
